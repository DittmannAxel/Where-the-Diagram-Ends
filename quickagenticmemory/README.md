...
The live path is intentionally staged: deploy the foundation and isolated identities first; explicitly deploy the paid Fabric/Foundry platform; publish one approved commit to Fabric; configure GitHub source access and the published Foundry application's distinct identity; then deploy the allowlisted Container App and attach the MCP-enabled agent version. OneLake publication uses a unique temporary directory, read-back byte and SHA-256 verification, and an atomic no-replace rename. The repository also creates the Fabric Workspace, Lakehouse, Graph Model, and Notebook through public APIs, generates the canonical Graph definition from the checked-in contract, and completes the official on-demand `RefreshGraph` job before accepting GQL evidence.
