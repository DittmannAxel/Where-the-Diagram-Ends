...
def execute_smoke(
    ...
    *,
    ...
    return {
        "receiptVersion": "qam-foundry-smoke/1.0",
        "status": "passed",
...
def _tool_output_object(value: Any, label: str) -> dict[str, Any]:
    if isinstance(value, str):
        parsed_values = _json_object_sequence(value, label)
        parsed = parsed_values[0]
        if len(parsed_values) == 2:
            envelope = parsed_values[1]
            envelope_keys = set(envelope)
            if envelope_keys == {"structuredResponse"}:
                structured_response = envelope["structuredResponse"]
            elif envelope_keys == {"structured_response"}:
                structured_response = envelope["structured_response"]
            else:
                raise SmokeAssertionError(f"{label} has an unsupported second JSON object")
            if not isinstance(structured_response, dict) or structured_response != parsed:
                raise SmokeAssertionError(f"{label} structured response does not match its JSON content")
            return structured_response
        if len(parsed_values) != 1:
            raise SmokeAssertionError(f"{label} contains too many JSON objects")
    else:
...
def _json_object_sequence(value: str, label: str) -> list[dict[str, Any]]:
    """Decode the exact JSON sequence emitted by Foundry for MCP structured output."""
    ...
    while cursor < len(value):
        ...
        if not isinstance(item, dict):
            raise SmokeAssertionError(f"{label} JSON sequence contains a non-object")
        parsed.append(item)
        if len(parsed) > 2:
            raise SmokeAssertionError(f"{label} contains too many JSON objects")
    if not parsed:
