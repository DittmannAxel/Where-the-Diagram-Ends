...
CALLER_IDENTITY_SOURCE = "FoundryProject.identity.systemAssigned"
...
_GRAPH_PROPAGATION_TIMEOUT_SECONDS = 60.0
_GRAPH_PROPAGATION_RETRYABLE_STATUSES = {404, 408, 429, 500, 502, 503, 504}
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--connection-name", default=os.getenv("QAM_FOUNDRY_CONNECTION_NAME", "qam-mcp-project-identity")
    )
...
def create_published_identity(
    ...
    """Publish an inert app while deriving the project MI used by RemoteTool."""
    ...
    project_identity = _project_managed_identity(config, arm)
    if _identity_dimensions_overlap(identity, project_identity):
        raise RuntimeError("Agent Application and Foundry project must have distinct identities")
    ...
    stable_application = _wait_for_application(arm, config, timeout_seconds=0)
    if _application_identity(stable_application) != identity:
        raise RuntimeError("Agent Application identity changed while publishing its initial deployment")
    if _project_managed_identity(config, arm) != project_identity:
        raise RuntimeError("Foundry project managed identity changed while publishing the application")

    client_id, principal_id = identity
    caller_client_id, caller_principal_id = project_identity
    return {
        ...
        "projectManagedIdentityClientId": caller_client_id,
        "projectManagedIdentityPrincipalId": caller_principal_id,
        "projectManagedIdentitySource": CALLER_IDENTITY_SOURCE,
        ...
        "accessContract": {
            ...
            "callerClientId": caller_client_id,
            "callerPrincipalId": caller_principal_id,
            "allowedClientApplicationIds": [caller_client_id],
            "allowedPrincipalIds": [caller_principal_id],
...
def attach_mcp(
    ...
    expected_caller = _registration_caller_identity(registration, config)
    access = _access_receipt_contract(access_receipt, registration, config)
    _verify_attach_access_gate(config, expected, expected_caller, access, arm)
    ...
    if _project_managed_identity(config, arm) != expected_caller:
        raise RuntimeError("Foundry project managed identity changed while updating the deployment")

    client_id, principal_id = expected
    caller_client_id, caller_principal_id = expected_caller
    return {
        "agentName": config.agent_name,
        "agentVersion": version,
        "projectResourceId": config.project_resource_id,
...
def _verify_attach_access_gate(
    config: FoundryConfig,
    expected_application: tuple[str, str],
    expected_caller: tuple[str, str],
    access: dict[str, str],
    client: AzureJsonClient,
) -> None:
    application = _wait_for_application(client, config, timeout_seconds=0)
    if _application_identity(application) != expected_application:
        raise ValueError("live Agent Application identity differs from the access registration")
    _verify_live_access(config, expected_application, expected_caller, access, client)
...
def _access_receipt_contract(
    ...
    application_client_id, application_principal_id = _registration_identity(registration, config)
    caller_client_id, caller_principal_id = _registration_caller_identity(registration, config)
    ...
    expected_text = {
        ...
        "requiredAppRole": "Qam.Read",
        "applicationIdentitySource": APPLICATION_IDENTITY_SOURCE,
        "callerIdentitySource": CALLER_IDENTITY_SOURCE,
        "callerIdentityType": CALLER_IDENTITY_TYPE,
    }
    ...
    for field, expected in uuid_fields.items():
        ...
        if actual != _normalized_uuid(expected):
            raise ValueError(f"access receipt {field} does not match its registered identity")
    ...
    if receipt.get("mcpRequestedAccessTokenVersion") != 2:
        raise ValueError("access receipt does not require MCP access-token version 2")
    if not _uuid_list_matches(receipt.get("allowedClientApplicationIds"), [caller_client_id]):
        raise ValueError("access receipt client allowlist does not match the Foundry project identity")
    if not _uuid_list_matches(receipt.get("allowedPrincipalIds"), [caller_principal_id]):
        raise ValueError("access receipt principal allowlist does not match the Foundry project identity")
...
def _verify_live_access(
    config: FoundryConfig,
    application_identity: tuple[str, str],
    caller_identity: tuple[str, str],
    access: dict[str, str],
    client: AzureJsonClient,
) -> None:
    """Re-read Graph and Container Apps state immediately before an attach mutation."""
    application_client_id, application_principal_id = application_identity
    caller_client_id, caller_principal_id = caller_identity
    if _identity_dimensions_overlap(application_identity, caller_identity):
        raise ValueError("Agent Application and Foundry project caller identities must remain distinct")
    if _project_managed_identity(config, client) != caller_identity:
        raise ValueError("live Foundry project managed identity differs from the access registration")
    api_principal_id = access["mcpApiPrincipalId"]
    api_application = client.request(
        "GET",
        (
            "https://graph.microsoft.com/v1.0/applications/"
            f"{access['mcpApiApplicationObjectId']}?$select=id,appId,appRoles,api"
        ),
        scope=GRAPH_SCOPE,
    )
    ...
    if (
        ...
        or not isinstance(api_application.get("api"), dict)
        or api_application["api"].get("requestedAccessTokenVersion") != 2
    ):
        raise ValueError("live MCP API application does not match the exact Qam.Read/v2-token contract")
    ...
    application_service_principal = client.request(
        "GET",
        (f"https://graph.microsoft.com/v1.0/servicePrincipals/{application_principal_id}?$select=id,appId"),
        scope=GRAPH_SCOPE,
    )
    if _normalized_uuid(application_service_principal.get("id")) != _normalized_uuid(
        application_principal_id
    ) or _normalized_uuid(application_service_principal.get("appId")) != _normalized_uuid(
    ...
    assignment = client.request(
        ...
        (
            "https://graph.microsoft.com/v1.0/servicePrincipals/"
            f"{caller_principal_id}/appRoleAssignments/{access['qamReadAssignmentId']}"
    ...
    expected_assignment = {
        ...
        "principalId": _normalized_uuid(caller_principal_id),
    ...
    _verify_auth_config(auth_config, config, caller_identity, access)
...
def _registration_identity(registration: dict[str, Any], config: FoundryConfig) -> tuple[str, str]:
    ...
    if registration.get("applicationIdentitySource") != APPLICATION_IDENTITY_SOURCE:
        raise ValueError("registration has an unsupported Agent Application identity source")
...
def _registration_caller_identity(registration: dict[str, Any], config: FoundryConfig) -> tuple[str, str]:
    """Return only the exact project MI that the RemoteTool connection will use."""
    application_identity = _registration_identity(registration, config)
    if registration.get("projectManagedIdentitySource") != CALLER_IDENTITY_SOURCE:
        raise ValueError("registration has an unsupported Foundry project identity source")
    caller_client_id = _normalized_uuid(registration.get("projectManagedIdentityClientId"))
    caller_principal_id = _normalized_uuid(registration.get("projectManagedIdentityPrincipalId"))
    if caller_client_id is None or caller_principal_id is None:
        raise ValueError("registration has no complete Foundry project managed identity")
    caller_identity = (caller_client_id, caller_principal_id)
    if _identity_dimensions_overlap(application_identity, caller_identity):
        raise ValueError("registration does not keep Agent Application and project identities distinct")

    contract = registration.get("accessContract")
    valid = (
        isinstance(contract, dict)
        and contract.get("requiredAppRole") == "Qam.Read"
        and contract.get("identitySource") == CALLER_IDENTITY_SOURCE
        and contract.get("callerIdentityType") == CALLER_IDENTITY_TYPE
        and _normalized_uuid(contract.get("callerClientId")) == caller_client_id
        and _normalized_uuid(contract.get("callerPrincipalId")) == caller_principal_id
        and _uuid_list_matches(contract.get("allowedClientApplicationIds"), [caller_client_id])
        and _uuid_list_matches(contract.get("allowedPrincipalIds"), [caller_principal_id])
    )
    if not valid:
        raise ValueError("registration does not bind Qam.Read to the Foundry project managed identity")
    return caller_identity


def _project_managed_identity(
    config: FoundryConfig,
    client: AzureJsonClient,
    *,
    timeout_seconds: float = _GRAPH_PROPAGATION_TIMEOUT_SECONDS,
    poll_interval_seconds: float = _POLL_INTERVAL_SECONDS,
) -> tuple[str, str]:
    """Resolve a Foundry project's system identity via ARM and its Entra SP via Graph."""
    project = client.request(
        "GET",
        (f"https://management.azure.com{config.project_resource_id}?api-version={PROJECT_API_VERSION}"),
        scope=ARM_SCOPE,
    )
    ...
    if (
        ...
        or resource_id.rstrip("/").casefold() != config.project_resource_id.rstrip("/").casefold()
        or not isinstance(identity, dict)
        or identity.get("type") != "SystemAssigned"
        or _normalized_uuid(identity.get("tenantId")) is None
    ):
        raise ValueError("live Foundry project has no exact system-assigned identity")
    principal_id = _normalized_uuid(identity.get("principalId"))
    if principal_id is None:
        raise ValueError("live Foundry project has no exact system-assigned identity")
    graph_url = (
        "https://graph.microsoft.com/v1.0/servicePrincipals/"
        f"{principal_id}?$select=id,appId,servicePrincipalType"
    )
    deadline = time.monotonic() + timeout_seconds
    while True:
        try:
            service_principal = client.request("GET", graph_url, scope=GRAPH_SCOPE)
            break
        except AzureRequestError as error:
            if error.status_code not in _GRAPH_PROPAGATION_RETRYABLE_STATUSES or timeout_seconds <= 0:
                raise
            _wait_for_next_poll(
                deadline,
                poll_interval_seconds,
                "Foundry project managed identity did not propagate to Microsoft Graph within "
                "the bounded poll",
                error,
            )
    client_id = _normalized_uuid(service_principal.get("appId"))
    if (
        _normalized_uuid(service_principal.get("id")) != principal_id
        or client_id is None
        or service_principal.get("servicePrincipalType") != "ManagedIdentity"
    ):
        raise ValueError("live Foundry project identity has no matching managed-identity service principal")
    return client_id, principal_id


def _identity_dimensions_overlap(first: tuple[str, str], second: tuple[str, str]) -> bool:
    """Treat either reused client or principal dimension as an identity-boundary violation."""
    return first[0].casefold() == second[0].casefold() or first[1].casefold() == second[1].casefold()
