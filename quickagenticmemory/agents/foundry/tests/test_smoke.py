...
def test_accepts_foundry_concatenated_content_and_matching_structured_response() -> None:
    payload = successful_payload()
    output = payload["output"]
    assert isinstance(output, list)
    for event in output:
        content = json.loads(event["output"])
        event["output"] = json.dumps(content) + "\n" + json.dumps({"structuredResponse": content})
...
@pytest.mark.parametrize("second", [{"unexpected": {}}, {"structuredResponse": {"different": True}}])
def test_rejects_untrusted_or_mismatched_second_tool_output_object(
    second: dict[str, object],
) -> None:
    payload = successful_payload()
    output = payload["output"]
    assert isinstance(output, list)
    output[0]["output"] = f"{output[0]['output']}\n{json.dumps(second)}"
...
def test_execute_bounds_calls_and_emits_only_evidence_summary() -> None:
    ...
    assert summary["receiptVersion"] == "qam-foundry-smoke/1.0"
