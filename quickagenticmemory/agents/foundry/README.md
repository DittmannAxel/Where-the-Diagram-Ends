...
any underlying version `instance_identity`; only the distinct application identity is granted
`Qam.Read` and admitted by Container Apps EasyAuth.

## Identity and call chain

There are three deliberately separate identities:

1. **Outbound Agent Application identity** — Foundry's
   `properties.defaultInstanceIdentity.clientId/principalId`; it obtains a token for the exact QAM
   audience and calls `/mcp`.
2. **Inbound invoker identity** — the human, CI service principal, or managed identity that calls
   the published Responses endpoint; it needs `Foundry User` at the individual Agent Application
   scope.
3. **QAM Container App runtime identity** — accesses Fabric, Key Vault, and monitoring. It is not
   allowed to invoke the published application and is not the inbound MCP caller.
...
2. An administrator runs `configure-access.sh`. The script reads the live application resource,
   verifies both identity IDs against the registration artifact, grants `Qam.Read`, verifies the
   resulting assignment, and emits a non-secret access receipt bound to the planned Container App.
3. The QAM Container App is redeployed with both the application client ID and principal ID in its
   EasyAuth allowlists.
4. `attach` requires that receipt and, before each connection/version/deployment mutation,
   ...
   create the secretless `AgenticIdentityToken` RemoteTool connection, create the MCP-enabled agent
...
The tool-less first deployment prevents any underlying development identity from making a
downstream QAM call before the distinct published identity exists. The application cannot grant
itself access.
...
The artifact contains identifiers, never credentials. It records the application resource ID,
the distinct client/principal pair, and the exact downstream access contract. The `.artifacts`
...
a mismatched audience, a mismatched allowlist, or an identity pair that differs from the live Agent
Application. It then invokes the shared `bootstrap-mcp-entra.sh`, verifies the exact enabled
...
EasyAuth checks audience, token application ID, and token object ID. The MCP API independently
requires `Qam.Read`.
...
concrete app-role assignment must bind the published application's principal to that exact role and
MCP API principal. The
...
exactly the configured API client, audience, one allowed application client ID, and one allowed
principal ID. Missing read permission or any mismatch fails closed. The attach operator therefore
...
- No model key, GitHub token, MCP bearer token, client secret, or certificate is stored. Local
  ...
  token for the Agent Application identity and exact audience.
- The RemoteTool connection is a project resource. Other development agents see project
  resources, but this flow grants neither a legacy shared project identity nor a new-model agent
  version identity. Other published applications have different identities and also fail the
  prescribed exact EasyAuth/`Qam.Read` gate.
...
- Fabric is accessed by the QAM runtime identity, not by the Foundry agent. The agent receives only
  bounded graph/read results through MCP.
...
coupling, stable application/deployment payloads, published-identity and access-receipt binding,
fail-before-mutation role/EasyAuth checks, rejection of shared project identity shapes, redirect
