...
export interface HttpServerOptions {
  ...
  readonly requestLogger?: (completion: HttpRequestCompletion) => void;
}
...
export interface HttpRequestCompletion {
  ...
  readonly trustedIdentityHeaderPresent: boolean;
...
function defaultRequestLogger(completion: HttpRequestCompletion): void {
  console.error("MCP HTTP request completed:", JSON.stringify(completion));
}
...
export async function startHttpServer(adapters: GatewayAdapters, options: HttpServerOptions): Promise<RunningHttpServer> {
  ...
  const requestLogger = options.requestLogger ?? defaultRequestLogger;

  const server = createServer((req, res) => {
    const started = process.hrtime.bigint();
    const trustedHeaderName =
      options.auth.mode === "trusted-header" ? options.auth.headerName : "x-ms-client-principal-id";
    const completionBase = {
      method: req.method ?? "UNKNOWN",
      path: requestPath(req),
      trustedIdentityHeaderPresent: headerPresent(req.headers[trustedHeaderName]),
      originPresent: headerPresent(req.headers.origin),
    };
    ...
    const logCompletion = (): void => {
      ...
      try {
        requestLogger({ ...completionBase, status: res.statusCode, durationMs });
      } catch {
        console.error("MCP HTTP request completion logger failed");
      }
    };
