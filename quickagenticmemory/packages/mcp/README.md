...
Each completed HTTP request also emits only method, URL path without query, status, duration, and
booleans indicating whether an Origin or trusted identity header was present. Header values, query
values, and bodies are never included.
