...
- `../scripts/publish-industrial-fabric.sh` orchestrates immutable OneLake publication, the checked-in Notebook, generated Graph definition update, the commit-pinned live GQL gate, and—only when explicitly selected—the post-GQL definition-updater cleanup.
...
5. run `publish-industrial-fabric.sh` for the approved projection. It stages immutable OneLake data, runs the reviewed Notebook, applies the generated canonical Graph Model definition, and requires bounded GQL to return the exact manifest repository, projection ID, commit, and counts. This Preview data/model gate occurs before ACA exists. Then use its explicit cleanup flags or run `finalize-fabric-definition-updater.sh` separately under workspace Admin to prove that the temporary updater is Viewer;
...
2. run `publish-industrial-fabric.sh`, which composes the following existing fail-closed steps in order:
   ...
   5. only when `--acceptance-cleanup` and the exact updater service-principal object ID were supplied, `finalize-fabric-definition-updater.sh` runs after GQL and emits a receipt after one exact Viewer assignment is visible through both list and item APIs.
