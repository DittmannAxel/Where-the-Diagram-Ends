...
- `../scripts/refresh-fabric-graph.sh` starts the Preview GraphModel `RefreshGraph` job, validates its exact same-origin Core job-instance `Location`, honors bounded `Retry-After`, and applies both per-request and overall timeouts before emitting a terminal-success receipt.
...
- `../scripts/build-cloud-image.sh` asks ACR to build from one exact public GitHub URL plus full commit SHA, polls the run to a terminal state, locks the resolved manifest, and emits a non-secret JSON receipt. It does not require local Docker. `build-push-image.sh` remains a separate local-development helper.
...
- `../scripts/grant-fabric-access.sh` defaults the runtime and protected GQL-smoke identities to Viewer, as documented by Fabric. The industrial path explicitly selects runtime Contributor as a recorded managed-identity Graph Preview compatibility workaround; an explicitly selected definition updater also receives Contributor.
- `../scripts/finalize-fabric-definition-updater.sh` is the explicit, workspace-Admin acceptance cleanup for that temporary definition updater. It atomically patches one exact Contributor assignment to Viewer, accepts an already-Viewer assignment idempotently, and fails closed on missing, duplicate, malformed, Member, or Admin state.
...
5. Fabric currently documents Viewer as sufficient for Graph queries by managed identities and service principals. During this PoC's cloud diagnosis, the same image/query returned GQL `42000` with the newly granted runtime Viewer and `00000` with a previously propagated Contributor identity. The industrial orchestrator therefore explicitly passes `--runtime-role Contributor` as a Preview compatibility workaround and still requires the final runtime identity itself to pass GQL. The protected deployment identity remains Viewer for normal GQL smoke, or explicitly Contributor while it publishes the generated Graph definition. `grant-fabric-access.sh` updates one unambiguous runtime Viewer assignment in place, never downgrades a broader role, and fails closed on duplicate assignments; neither identity can self-grant. Re-test Viewer and remove this workaround when the same runtime acceptance test passes.
6. Generate a deterministic projection for one approved Git commit, then run `publish-industrial-fabric.sh` to stage it immutably, load the Delta tables, apply the canonical definition, complete the official on-demand `RefreshGraph` job, and require live GQL to return the manifest's exact repository, projection ID, commit, and node/edge counts before ACA deployment.
7. After the commit-pinned live GQL gate succeeds, run the explicit definition-updater acceptance cleanup and retain its verified `qam-fabric-role-cleanup/1.0` receipt. Workspace Member/Admin and tenant-setting changes remain administrator operations.
...
`--prepare-only` creates no credential and grants no caller. After Foundry publishes its distinct Agent Application identity and derives the project's system-assigned identity, `configure-access.sh` reruns the same idempotent reconciliation with the project identity as the exact caller and assigns only `Qam.Read`.

For the Microsoft Foundry path, first publish the inert, tool-less version as an **Agent Application** through the tenant-registered `Microsoft.CognitiveServices` `2026-05-15-preview` application/deployment resources. Poll the live application, record its distinct `properties.defaultInstanceIdentity`, and separately resolve the exact Foundry project system identity through ARM and Graph. The secretless RemoteTool connection uses `ProjectManagedIdentity`: grant only that project client/principal pair `Qam.Read` and place it in both Container Apps EasyAuth allow-lists before attaching the MCP-enabled version. `allowedPrincipals` must contain only `identities`; even an empty `groups` member is rejected because it activates a different EasyAuth authorization path. The Agent Application identity remains distinct and ungranted. The Azure runtime UAMI emitted by this template is **not** that caller; it is used only for Fabric, Key Vault, and telemetry access inside Container Apps. The exact phased commands and identity verification are documented in [`../agents/foundry/README.md`](../agents/foundry/README.md).
...
quickagenticmemory/scripts/publish-industrial-fabric.sh \
  ...
  --acceptance-cleanup \
  --definition-updater-principal-id '<protected-deployment-service-principal-object-id>'
```

The definition directory must be new or contain none of the four definition parts; the generator refuses to overwrite review evidence. `publish-industrial-fabric.sh` publishes immutable OneLake staging, runs the checked-in parameterized Notebook, generates and applies the canonical public Graph definition, starts the official on-demand `RefreshGraph` job, and requires that job to finish before polling bounded live GQL for the manifest's exact repository, projection ID, commit, and node/edge counts. Graph Model creation, definition management, refresh, and execute-query remain Preview/Beta surfaces. Cleanup is never implicit: both cleanup arguments are required, and the Admin-only role change starts only after that live GQL gate. If the cleanup fails, the command reports that publication already succeeded and exits nonzero instead of hiding the broader role.

The same cleanup can be rerun idempotently as a separately governed acceptance step:

```bash
quickagenticmemory/scripts/finalize-fabric-definition-updater.sh \
  --workspace-id "${fabric_workspace_id}" \
  --definition-updater-principal-id '<protected-deployment-service-principal-object-id>'
```
...
3. configure only the versionless secret URI as a protected GitHub Environment variable and, under an operator allowed to read that secret, run the complete App-mode check below (or the corresponding `token`/`none` form documented under **GitHub content authentication**):
   ...
   quickagenticmemory/scripts/validate-github-access.sh \
     ...
     --live
   ```
4. either the complete industrial orchestrator has already applied the Fabric roles, or a Fabric workspace Admin runs `grant-fabric-access.sh --runtime-principal-id '<runtime-uami>' --runtime-role Contributor --smoke-principal-id '<github-oidc-principal>'`; this explicitly enables the observed runtime Preview workaround, while the separate smoke identity remains Viewer unless selected for definition publication;
5. run `publish-industrial-fabric.sh` for the approved projection. It stages immutable OneLake data, runs the reviewed Notebook, applies the generated canonical Graph Model definition, completes the bounded official `RefreshGraph` job, and requires bounded GQL to return the exact manifest repository, projection ID, commit, and counts. This Preview data/model gate occurs before ACA exists. Then use its explicit cleanup flags or run `finalize-fabric-definition-updater.sh` separately under workspace Admin to prove that the temporary updater is Viewer;
...
The protected workflow relies on the Repository Writer assignment created by the reviewed administrator phase and never grants itself RBAC. For a separately authorized one-off operator test, `build-cloud-image.sh` can create one short-lived Repository Writer assignment when both `--temporary-writer-principal-id` and `--temporary-writer-principal-type` identify the current Azure CLI caller. The helper preselects the assignment ID, keeps it through the terminal ACR run, removes that exact ID on success or failure through an `EXIT` trap, and fails unless its absence is verified. This optional mode requires the operator to already hold `Microsoft.Authorization/roleAssignments/write`; it does not broaden the GitHub workflow identity.

An existing image tag is never overwritten. Reuse requires `--expected-existing-digest sha256:...`, an exact digest match, and live `writeEnabled=false` plus `deleteEnabled=false` attributes. The receipt binds the public Git URL, derived `owner/repository`, full commit, fixed Dockerfile, tag, digest, terminal run status, and temporary-writer cleanup result without including tokens or principal/resource IDs.
...
4. Submit an ACR cloud build from the exact public `https://github.com/OWNER/REPOSITORY.git#FULL_SHA` source using the fixed repository-root `quickagenticmemory/packages/mcp/Dockerfile`; no local Docker daemon participates.
5. Poll that exact ACR run within fixed bounds, resolve its `sha256` manifest digest, and lock the commit-SHA-tagged manifest against write and delete.
6. Run the deployment again using `<registry>/<repository>@sha256:...`.
7. Test public `/healthz` and anonymous `/mcp` rejection.

Illustrative operator-shell sequence:
...
source_git_url='https://github.com/<owner>/<repository>.git'
source_commit_sha='<full-lowercase-40-character-git-sha>'
image_receipt="$(quickagenticmemory/scripts/build-cloud-image.sh \
  --registry "${acr_name}" \
  --source-git-url "${source_git_url}" \
  --source-ref "${source_commit_sha}" \
  --image-tag "${source_commit_sha}")"
...
2. run `publish-industrial-fabric.sh`, which composes the following existing fail-closed steps in order:
   ...
   4. `refresh-fabric-graph.sh` starts `POST /v1/workspaces/{workspace}/graphModels/{graph}/jobs/refreshGraph/instances`, follows only the returned exact same-origin Core job-instance URL, and fails unless bounded polling reaches `Completed` or `Succeeded` within its hard deadline;
   5. `smoke-test-fabric-graph.sh` retries bounded read-only GQL until it returns the exact repository, projection ID, commit, and node/edge counts from the immutable manifest;
   6. only when `--acceptance-cleanup` and the exact updater service-principal object ID were supplied, `finalize-fabric-definition-updater.sh` runs after GQL and emits a receipt after one exact Viewer assignment is visible through both list and item APIs.
...
After the foundation exists, an approved Fabric workspace Admin can reconcile the complete access contract below. A Member can add a missing lower role, but only Admin can update an existing Viewer assignment to Contributor:
...
Omit `--runtime-role Contributor` to keep Fabric's documented Viewer default. The industrial path includes it because the same live image/query returned GQL `42000` with newly granted runtime Viewer and `00000` with a previously propagated Contributor identity. Treat this as a Preview compatibility workaround, require a final GQL pass from the exact runtime identity, and periodically retry that acceptance with Viewer. If the runtime already has exactly one Viewer assignment, the script uses the documented update endpoint to change that exact assignment in place; duplicate, malformed, or unexpected assignments fail before any mutation, and broader roles are never downgraded. Updating an existing assignment requires workspace Admin, while adding a new lower role permits Member or higher.

Add `--definition-updater-principal-id '<github-oidc-object-id>'` only if the workflow will apply a generated or separately reviewed definition. When that ID matches the smoke principal, the script assigns Contributor instead of creating a redundant Viewer assignment. The separate smoke identity otherwise keeps Viewer; after successful acceptance, use `finalize-fabric-definition-updater.sh` to reconcile that exact temporary Contributor to Viewer. The scripts use only Microsoft's documented [list](https://learn.microsoft.com/en-us/rest/api/fabric/core/workspaces/list-workspace-role-assignments), [get](https://learn.microsoft.com/en-us/rest/api/fabric/core/workspaces/get-workspace-role-assignment), [add](https://learn.microsoft.com/en-us/rest/api/fabric/core/workspaces/add-workspace-role-assignment), and [update](https://learn.microsoft.com/en-us/rest/api/fabric/core/workspaces/update-workspace-role-assignment) workspace-role endpoints. They follow only same-origin pagination URLs, reject an incomplete continuation contract, validate create/update responses, and perform bounded `Retry-After`-aware reads plus a complete final live re-list. No receipt is emitted unless every target has exactly one well-formed assignment meeting its requested contract. The operator token needs delegated `Workspace.ReadWrite.All`; this administrator bootstrap cannot be delegated to the runtime or smoke identity.
...
The generic MCP smoke proves the public health exception and that an anonymous `/mcp` request returns exactly `401`. The industrial cloud acceptance additionally passed an authenticated Foundry Agent Application call through the `ProjectManagedIdentity` RemoteTool connection to the MCP gateway, Fabric Graph, and commit-pinned public GitHub content. That identity-bound gate must still be rerun in every tenant; the public [redacted evidence](../tests/industrial-component-obsolescence/screens/) deliberately omits tenant and resource identifiers.
...
- [Run an on-demand Graph refresh job](https://learn.microsoft.com/en-us/rest/api/fabric/graphmodel/background-jobs/run-on-demand-refresh-graph)
- [Get a Fabric item job instance](https://learn.microsoft.com/en-us/rest/api/fabric/core/job-scheduler/get-item-job-instance)
