...
qam_info "validating the identity-only Container Apps EasyAuth contract"
if grep -Eq '^[[:space:]]*groups[[:space:]]*:' "${QAM_INFRA_DIR}/modules/container-app.bicep"; then
  qam_fail "Container Apps EasyAuth allowedPrincipals must not enable group authorization"
fi
jq -e '
  ([.. | objects | select(.type? == "Microsoft.App/containerApps/authConfigs")] | length) == 1 and
  ([.. | objects | select(.type? == "Microsoft.App/containerApps/authConfigs")][0]
    .properties.identityProviders.azureActiveDirectory.validation.defaultAuthorizationPolicy.allowedPrincipals
    | keys) == ["identities"]
' "${validation_dir}/main.json" >/dev/null \
  || qam_fail "compiled Container Apps EasyAuth must allow only the exact caller identities"
...
for script_name in \
  ...
  finalize-fabric-definition-updater.sh \
  grant-fabric-access.sh \
  generate-fabric-graph-definition.sh \
  publish-industrial-fabric.sh \
  refresh-fabric-graph.sh \
...
grep -Fq -- '--runtime-role Contributor' "${industrial_orchestrator}" \
  || qam_fail "industrial orchestrator must explicitly select the recorded Graph Preview runtime workaround"
...
publication_refresh_line="$(grep -nF "\${QAM_SCRIPTS_DIR}/refresh-fabric-graph.sh" "${fabric_publication}" | head -1 | cut -d: -f1)"
publication_gql_line="$(grep -nF "\${QAM_SCRIPTS_DIR}/smoke-test-fabric-graph.sh" "${fabric_publication}" | head -1 | cut -d: -f1)"
publication_cleanup_line="$(grep -nF "\${QAM_SCRIPTS_DIR}/finalize-fabric-definition-updater.sh" "${fabric_publication}" | head -1 | cut -d: -f1)"
[ -n "${publication_onelake_line}" ] \
  ...
  && [ "${publication_update_line}" -lt "${publication_refresh_line}" ] \
  && [ "${publication_refresh_line}" -lt "${publication_gql_line}" ] \
  && [ "${publication_gql_line}" -lt "${publication_cleanup_line}" ] \
  || qam_fail "Fabric publication must stage OneLake, run the Notebook, apply the definition, complete RefreshGraph, then query the graph"
fabric_graph_refresh="${QAM_SCRIPTS_DIR}/refresh-fabric-graph.sh"
grep -Fq "/graphModels/\${graph_model_id}/jobs/refreshGraph/instances" "${fabric_graph_refresh}" \
  || qam_fail "Fabric Graph refresh must call the official GraphModel RefreshGraph endpoint"
grep -Fq "/items/\${graph_model_id}/jobs/instances/" "${fabric_graph_refresh}" \
  || qam_fail "Fabric Graph refresh must poll the exact Core item job-instance endpoint"
if ! grep -Fq -- "--connect-timeout \"\${connect_timeout_seconds}\"" "${fabric_graph_refresh}" \
  || ! grep -Fq -- "--max-time \"\${call_timeout}\"" "${fabric_graph_refresh}"; then
  qam_fail "Fabric Graph refresh must enforce per-request connection and total timeouts"
fi
if ! grep -Fq -- '--acceptance-cleanup' "${fabric_publication}" \
  || ! grep -Fq -- \
    "--definition-updater-principal-id \"\${definition_updater_principal_id}\"" \
    "${fabric_publication}"; then
  qam_fail "Fabric definition-updater cleanup must remain an explicit post-GQL publisher option"
fi

fabric_definition_cleanup="${QAM_SCRIPTS_DIR}/finalize-fabric-definition-updater.sh"
if ! grep -Fq -- '--request PATCH' "${fabric_definition_cleanup}" \
  || ! grep -Fq -- "patch_body='{\"role\":\"Viewer\"}'" \
    "${fabric_definition_cleanup}"; then
  qam_fail "Fabric definition-updater cleanup must use the documented atomic Viewer PATCH"
fi
if grep -Eq -- '--request (POST|DELETE)' "${fabric_definition_cleanup}"; then
  qam_fail "Fabric definition-updater cleanup must never create or delete a role assignment"
fi
...
expect_cli_failure "industrial identity separation is not UUID-case-insensitive" \
  "${industrial_orchestrator}" \
  --resource-group qam-platform-contract \
  --fabric-admin-member fabric-admin@example.invalid \
  --operator-principal-id 11111111-1111-4111-8111-111111111111 \
  --runtime-principal-id aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa \
  --deployment-principal-id AAAAAAAA-AAAA-4AAA-8AAA-AAAAAAAAAAAA
...
expect_cli_failure "Fabric publication accepts cleanup without an exact updater principal" \
  "${fabric_publication}" \
  --acceptance-cleanup
expect_cli_failure "Fabric publication accepts an updater principal without explicit cleanup consent" \
  "${fabric_publication}" \
  --definition-updater-principal-id 22222222-2222-4222-8222-222222222222
...
image_build_line="$(grep -n 'name: Build immutable image in ACR from exact public commit' "${deploy_workflow}" | cut -d: -f1)"
...
if ! grep -Fq 'quickagenticmemory/scripts/build-cloud-image.sh' <<< "${cloud_build_block}" \
  ...
  || ! grep -Fq '.contractVersion == "qam-acr-cloud-build/1.0"' <<< "${cloud_build_block}" \
  || ! grep -Fq '.source.commitSha == $commitSha' <<< "${cloud_build_block}" \
  || ! grep -Fq '.temporaryWriter.requested == false' <<< "${cloud_build_block}" \
  || ! grep -Fq '.temporaryWriter.selfCreated == false' <<< "${cloud_build_block}"; then
  qam_fail "protected deploy workflow must gate ACA on the exact public, OIDC-authorized ACR cloud-build receipt"
fi
if grep -Fq 'build-push-image.sh' <<< "${cloud_build_block}" \
  || grep -Fq -- '--temporary-writer-principal-id' <<< "${cloud_build_block}"; then
  qam_fail "protected deploy workflow must neither use local Docker nor self-grant a temporary ACR writer"
fi
grep -Fq -- '--runtime-role Contributor --smoke-principal-id' "${deploy_workflow}" \
  || qam_fail "workflow administrator handoff must explicitly select the recorded runtime Preview workaround"
grep -Fq 'Microsoft documents Viewer for Graph query' "${deploy_workflow}" \
  || qam_fail "workflow handoff must disclose that runtime Contributor is an observed Preview workaround"
...
qam_info "running ACR cloud-build security and cleanup tests"
"${QAM_SCRIPTS_DIR}/tests/build-cloud-image.sh"

qam_info "running Fabric access role tests"
"${QAM_SCRIPTS_DIR}/tests/fabric-access.sh"

qam_info "running Fabric definition-updater acceptance-cleanup tests"
"${QAM_SCRIPTS_DIR}/tests/fabric-definition-updater-cleanup.sh"

qam_info "running Fabric Graph RefreshGraph tests"
"${QAM_SCRIPTS_DIR}/tests/fabric-graph-refresh.sh"
