...
mcp_entra_bootstrap="${QAM_SCRIPTS_DIR}/bootstrap-mcp-entra.sh"
grep -Fq "(.api // {}) | .requestedAccessTokenVersion = 2 | {api: .}" "${mcp_entra_bootstrap}" \
  || qam_fail "MCP Entra bootstrap must preserve the existing Graph API subfields"
api_contract_fixture='{"api":{"oauth2PermissionScopes":[{"id":"11111111-1111-4111-8111-111111111111","value":"existing"}],"preAuthorizedApplications":[{"appId":"22222222-2222-4222-8222-222222222222"}],"requestedAccessTokenVersion":1}}'
api_contract_patch="$(jq -c '(.api // {}) | .requestedAccessTokenVersion = 2 | {api: .}' <<< "${api_contract_fixture}")"
jq -e '
  .api.requestedAccessTokenVersion == 2 and
  .api.oauth2PermissionScopes == [{"id":"11111111-1111-4111-8111-111111111111","value":"existing"}] and
  .api.preAuthorizedApplications == [{"appId":"22222222-2222-4222-8222-222222222222"}]
' <<< "${api_contract_patch}" >/dev/null \
  || qam_fail "MCP Entra access-token reconciliation dropped an existing Graph API subfield"
...
# Reconcile an eventually consistent Qam.Read grant through the real bootstrap and access
# scripts with a stateful Azure CLI mock. The mock rejects the server-side filter so the scripts
# must follow every validated @odata.nextLink page. It then delays visibility independently for
# bootstrap and configure-access, and a second full run proves that no duplicate POST is emitted.
entra_reconciliation_dir="$(mktemp -d)"
entra_reconciliation_log="${entra_reconciliation_dir}/az.log"
entra_post_log="${entra_reconciliation_dir}/posts.log"
entra_sleep_log="${entra_reconciliation_dir}/sleeps.log"
entra_list_count_file="${entra_reconciliation_dir}/list-count"
entra_current_list_file="${entra_reconciliation_dir}/current-list"
entra_registration_file="${entra_reconciliation_dir}/registration.json"
: > "${entra_reconciliation_log}"
: > "${entra_post_log}"
: > "${entra_sleep_log}"
printf '0\n' > "${entra_list_count_file}"

entra_subscription_id='123e4567-e89b-42d3-a456-426614174000'
fixture_mcp_client='123e4567-e89b-42d3-a456-426614174000'
fixture_mcp_application_object='823e4567-e89b-42d3-a456-426614174000'
fixture_mcp_service_principal='323e4567-e89b-42d3-a456-426614174000'
entra_role_id='423e4567-e89b-42d3-a456-426614174000'
entra_assignment_id='UxOIjjUXr0WvIe4TRFgqTY4z9Wu5KxpBtlEpoTGjw-A'
entra_project_client_id='923e4567-e89b-42d3-a456-426614174000'
entra_project_principal_id='a23e4567-e89b-42d3-a456-426614174000'
entra_agent_client_id='623e4567-e89b-42d3-a456-426614174000'
entra_agent_principal_id='223e4567-e89b-42d3-a456-426614174000'
entra_tenant_id='b23e4567-e89b-42d3-a456-426614174000'
entra_project_resource_id="/subscriptions/${entra_subscription_id}/resourceGroups/qam-rg/providers/Microsoft.CognitiveServices/accounts/qam-ai/projects/qam"
entra_application_resource_id="${entra_project_resource_id}/applications/qam-knowledge-application"
entra_container_app_id="/subscriptions/${entra_subscription_id}/resourceGroups/qam-rg/providers/Microsoft.App/containerApps/qam-green"

jq -cn \
  --arg projectResourceId "${entra_project_resource_id}" \
  --arg applicationResourceId "${entra_application_resource_id}" \
  --arg applicationClientId "${entra_agent_client_id}" \
  --arg applicationPrincipalId "${entra_agent_principal_id}" \
  --arg projectManagedIdentityClientId "${entra_project_client_id}" \
  --arg projectManagedIdentityPrincipalId "${entra_project_principal_id}" \
  --arg audience "api://${fixture_mcp_client}" '
  {
    ...
    mcpUrl: "https://qam.green.azurecontainerapps.io/mcp",
    mcpAudience: $audience,
    accessContract: {
      identitySource: "FoundryProject.identity.systemAssigned",
      callerIdentityType: "ProjectManagedIdentity",
      callerClientId: $projectManagedIdentityClientId,
      callerPrincipalId: $projectManagedIdentityPrincipalId,
      allowedClientApplicationIds: [$projectManagedIdentityClientId],
      allowedPrincipalIds: [$projectManagedIdentityPrincipalId],
      requiredAppRole: "Qam.Read"
    }
  }' > "${entra_registration_file}"

# shellcheck disable=SC2329 # exported into the Entra reconciliation scripts' Bash processes
sleep() {
  printf '%s\n' "$*" >> "${QAM_ENTRA_SLEEP_LOG}"
}
...
az() {
  ...
  for argument in "$@"; do
    case "${previous}" in
      --method) method="$(printf '%s' "${argument}" | tr '[:lower:]' '[:upper:]')" ;;
      --uri | --url) uri="${argument}" ;;
      --query) query="${argument}" ;;
    esac
    previous="${argument}"
  done
  printf '%s\n' "$*" >> "${QAM_ENTRA_RECONCILIATION_LOG}"

  case "${first}:${second}:${third}" in
    account:show:*) return 0 ;;
    ad:app:show)
      if [ "${query}" = 'appRoles' ]; then
        jq -cn --arg role "${QAM_ENTRA_ROLE_ID}" '[{
        ...
        jq -cn \
          --arg id "${QAM_ENTRA_API_APP_OBJECT_ID}" \
          --arg appId "${QAM_ENTRA_API_CLIENT_ID}" \
          --arg audience "api://${QAM_ENTRA_API_CLIENT_ID}" \
          --arg role "${QAM_ENTRA_ROLE_ID}" '{
            id: $id,
            appId: $appId,
    ...
    ad:sp:show)
      jq -cn \
        --arg id "${QAM_ENTRA_API_PRINCIPAL_ID}" \
        --arg appId "${QAM_ENTRA_API_CLIENT_ID}" '{id: $id, appId: $appId}'
      return 0
      ;;
    rest:*) ;;
    *) return 1 ;;
  ...
  if [ "${method}" = 'GET' ] && [[ "${uri}" == *"/applications/${QAM_ENTRA_API_APP_OBJECT_ID}?"* ]]; then
    jq -cn '{api: {requestedAccessTokenVersion: 2}}'
    return 0
  fi
  if [ "${method}" = 'GET' ] && [ "${uri}" = "https://management.azure.com${QAM_ENTRA_APPLICATION_RESOURCE_ID}?api-version=2026-05-15-preview" ]; then
    jq -cn \
      --arg client "${QAM_ENTRA_AGENT_CLIENT_ID}" \
      --arg principal "${QAM_ENTRA_AGENT_PRINCIPAL_ID}" '{properties: {
        provisioningState: "Succeeded",
        authorizationPolicy: {authorizationScheme: "Default"},
        agents: [{agentName: "qam-knowledge-agent"}],
  ...
  if [ "${method}" = 'GET' ] && [ "${uri}" = "https://management.azure.com${QAM_ENTRA_PROJECT_RESOURCE_ID}?api-version=2025-06-01" ]; then
    jq -cn \
      --arg id "${QAM_ENTRA_PROJECT_RESOURCE_ID}" \
      --arg tenant "${QAM_ENTRA_TENANT_ID}" \
      --arg principal "${QAM_ENTRA_PROJECT_PRINCIPAL_ID}" '{
        id: $id,
        identity: {type: "SystemAssigned", tenantId: $tenant, principalId: $principal}
      }'
    return 0
  fi
  if [ "${method}" = 'GET' ] && [[ "${uri}" == *"/servicePrincipals/${QAM_ENTRA_AGENT_PRINCIPAL_ID}?"* ]]; then
    jq -cn \
      --arg id "${QAM_ENTRA_AGENT_PRINCIPAL_ID}" \
      --arg appId "${QAM_ENTRA_AGENT_CLIENT_ID}" '{id: $id, appId: $appId}'
    return 0
  fi
  if [ "${method}" = 'GET' ] && [[ "${uri}" == *"/servicePrincipals/${QAM_ENTRA_PROJECT_PRINCIPAL_ID}?"* ]]; then
    if [ "${query}" = 'appId' ]; then
      printf '%s\n' "${QAM_ENTRA_PROJECT_CLIENT_ID}"
    else
      jq -cn \
        --arg id "${QAM_ENTRA_PROJECT_PRINCIPAL_ID}" \
        --arg appId "${QAM_ENTRA_PROJECT_CLIENT_ID}" '{
          id: $id,
          appId: $appId,
  ...
  if [ "${method}" = 'GET' ] && [[ "${uri}" == *"/servicePrincipals/${QAM_ENTRA_API_PRINCIPAL_ID}?"* ]] \
    && [[ "${uri}" != *'/appRoleAssignedTo?'* ]]; then
    if [ "${query}" = 'appRoleAssignmentRequired' ]; then
      printf 'true\n'
    else
      jq -cn \
        --arg id "${QAM_ENTRA_API_PRINCIPAL_ID}" \
  ...
  if [ "${method}" = 'GET' ] && [[ "${uri}" == *'/appRoleAssignedTo?'* ]] \
    && [[ "${uri}" == *"\$filter="* ]]; then
    printf 'FILTER_UNSUPPORTED\n' >> "${QAM_ENTRA_RECONCILIATION_LOG}"
    return 1
  fi
  if [ "${method}" = 'GET' ] && [[ "${uri}" == *'/appRoleAssignedTo?'* ]] \
    && [[ "${uri}" != *"\$skiptoken=two"* ]]; then
    list_count="$(( $(command cat "${QAM_ENTRA_LIST_COUNT_FILE}") + 1 ))"
    printf '%s\n' "${list_count}" > "${QAM_ENTRA_LIST_COUNT_FILE}"
    printf '%s\n' "${list_count}" > "${QAM_ENTRA_CURRENT_LIST_FILE}"
    if [ "${QAM_ENTRA_MOCK_MODE}" = 'hostile-next-link' ]; then
      next_link="https://attacker.example/appRoleAssignedTo?\$skiptoken=two"
    else
      next_link="https://graph.microsoft.com/v1.0/servicePrincipals/${QAM_ENTRA_API_PRINCIPAL_ID}/appRoleAssignedTo?\$select=id,principalId,resourceId,appRoleId&\$skiptoken=two"
    fi
    jq -cn \
      --arg next "${next_link}" \
      --arg resource "${QAM_ENTRA_API_PRINCIPAL_ID}" \
      --arg role '723e4567-e89b-42d3-a456-426614174000' '{
        value: [{
          id: "unrelatedAssignment",
          principalId: "723e4567-e89b-42d3-a456-426614174000",
          resourceId: $resource,
          appRoleId: $role
        }],
        "@odata.nextLink": $next
  ...
  if [ "${method}" = 'GET' ] && [[ "${uri}" == *"\$skiptoken=two"* ]]; then
    current_list="$(command cat "${QAM_ENTRA_CURRENT_LIST_FILE}")"
    case "${current_list}" in
      1 | 2 | 4) jq -cn '{value: []}' ;;
      *)
        jq -cn \
          --arg id "${QAM_ENTRA_ASSIGNMENT_ID}" \
          --arg principal "${QAM_ENTRA_PROJECT_PRINCIPAL_ID}" \
          --arg resource "${QAM_ENTRA_API_PRINCIPAL_ID}" \
          --arg role "${QAM_ENTRA_ROLE_ID}" '{value: [{
            id: $id,
            principalId: $principal,
            resourceId: $resource,
            appRoleId: $role
  ...
  if [ "${method}" = 'POST' ] && [ "${uri}" = "https://graph.microsoft.com/v1.0/servicePrincipals/${QAM_ENTRA_API_PRINCIPAL_ID}/appRoleAssignedTo" ]; then
    printf 'POST\n' >> "${QAM_ENTRA_POST_LOG}"
    return 0
  fi
  return 1
...
export -f az sleep
export QAM_ENTRA_RECONCILIATION_LOG="${entra_reconciliation_log}"
export QAM_ENTRA_POST_LOG="${entra_post_log}"
export QAM_ENTRA_SLEEP_LOG="${entra_sleep_log}"
export QAM_ENTRA_LIST_COUNT_FILE="${entra_list_count_file}"
export QAM_ENTRA_CURRENT_LIST_FILE="${entra_current_list_file}"
export QAM_ENTRA_MOCK_MODE='eventual'
export QAM_ENTRA_API_CLIENT_ID="${fixture_mcp_client}"
export QAM_ENTRA_API_APP_OBJECT_ID="${fixture_mcp_application_object}"
export QAM_ENTRA_API_PRINCIPAL_ID="${fixture_mcp_service_principal}"
export QAM_ENTRA_ROLE_ID="${entra_role_id}"
export QAM_ENTRA_ASSIGNMENT_ID="${entra_assignment_id}"
export QAM_ENTRA_PROJECT_CLIENT_ID="${entra_project_client_id}"
export QAM_ENTRA_PROJECT_PRINCIPAL_ID="${entra_project_principal_id}"
export QAM_ENTRA_AGENT_CLIENT_ID="${entra_agent_client_id}"
export QAM_ENTRA_AGENT_PRINCIPAL_ID="${entra_agent_principal_id}"
export QAM_ENTRA_TENANT_ID="${entra_tenant_id}"
export QAM_ENTRA_PROJECT_RESOURCE_ID="${entra_project_resource_id}"
export QAM_ENTRA_APPLICATION_RESOURCE_ID="${entra_application_resource_id}"

entra_receipt="$("${QAM_ROOT_DIR}/agents/foundry/configure-access.sh" \
  --registration "${entra_registration_file}" \
  --mcp-api-client-id "${fixture_mcp_client}" \
  --container-app-resource-id "${entra_container_app_id}")"
jq -e \
  --arg role "${entra_role_id}" \
  --arg assignment "${entra_assignment_id}" \
  --arg principal "${entra_project_principal_id}" '
    .receiptVersion == "qam-foundry-access/2.0" and
    .callerIdentityType == "ProjectManagedIdentity" and
    .qamReadAppRoleId == $role and
    .qamReadAssignmentId == $assignment and
    .allowedPrincipalIds == [$principal]
  ' <<< "${entra_receipt}" >/dev/null \
  || qam_fail "eventually consistent Qam.Read reconciliation emitted an invalid receipt"
[ "$(grep -c '^POST$' "${entra_post_log}")" -eq 1 ] \
  || qam_fail "initial Qam.Read reconciliation must POST exactly once"
[ "$(command cat "${entra_list_count_file}")" -ge 5 ] \
  || qam_fail "Qam.Read reconciliation did not exercise both bounded propagation polls"
[ "$(wc -l < "${entra_sleep_log}" | tr -d ' ')" -ge 2 ] \
  || qam_fail "Qam.Read reconciliation did not wait between eventually consistent reads"
grep -Fq "\$skiptoken=two" "${entra_reconciliation_log}" \
  || qam_fail "Qam.Read reconciliation did not follow the Microsoft Graph nextLink"

second_entra_receipt="$("${QAM_ROOT_DIR}/agents/foundry/configure-access.sh" \
  --registration "${entra_registration_file}" \
  --mcp-api-client-id "${fixture_mcp_client}" \
  --container-app-resource-id "${entra_container_app_id}")"
jq -e --arg assignment "${entra_assignment_id}" '.qamReadAssignmentId == $assignment' \
  <<< "${second_entra_receipt}" >/dev/null \
  || qam_fail "idempotent Qam.Read reconciliation changed the assignment receipt"
[ "$(grep -c '^POST$' "${entra_post_log}")" -eq 1 ] \
  || qam_fail "idempotent Qam.Read reconciliation emitted a duplicate POST"

export QAM_ENTRA_MOCK_MODE='hostile-next-link'
expect_failure "cross-origin Microsoft Graph app-role assignment nextLink" \
  "${QAM_ROOT_DIR}/agents/foundry/configure-access.sh" \
  --registration "${entra_registration_file}" \
  --mcp-api-client-id "${fixture_mcp_client}" \
  --container-app-resource-id "${entra_container_app_id}"
if grep -Fq 'attacker.example' "${entra_reconciliation_log}"; then
  qam_fail "Qam.Read reconciliation followed a cross-origin Microsoft Graph nextLink"
fi

unset -f az sleep
unset QAM_ENTRA_RECONCILIATION_LOG QAM_ENTRA_POST_LOG QAM_ENTRA_SLEEP_LOG
unset QAM_ENTRA_LIST_COUNT_FILE QAM_ENTRA_CURRENT_LIST_FILE QAM_ENTRA_MOCK_MODE
unset QAM_ENTRA_API_CLIENT_ID QAM_ENTRA_API_APP_OBJECT_ID QAM_ENTRA_API_PRINCIPAL_ID
unset QAM_ENTRA_ROLE_ID QAM_ENTRA_ASSIGNMENT_ID QAM_ENTRA_PROJECT_CLIENT_ID
unset QAM_ENTRA_PROJECT_PRINCIPAL_ID QAM_ENTRA_AGENT_CLIENT_ID QAM_ENTRA_AGENT_PRINCIPAL_ID
unset QAM_ENTRA_TENANT_ID QAM_ENTRA_PROJECT_RESOURCE_ID QAM_ENTRA_APPLICATION_RESOURCE_ID
rm -rf "${entra_reconciliation_dir}"
...
# Exercise the rendered handoff rather than depending on how its printf command
# happens to be wrapped across YAML lines. Trap stubs make any accidental command
# execution observable while allowing the intended Markdown text to be rendered.
handoff_test_dir="$(mktemp -d)"
handoff_script="${handoff_test_dir}/handoff.sh"
handoff_summary="${handoff_test_dir}/summary.md"
handoff_execution_log="${handoff_test_dir}/executed.log"
awk '
  $0 == "      - name: Write foundation administrator handoff" {
    in_step = 1
    next
  }
  in_step && $0 ~ /^      - name: / { exit }
  ...
  in_run && $0 ~ /^          / {
    sub(/^          /, "")
    gsub(/\$\{\{ inputs\.update_fabric_definition \}\}/, "false")
    print
  }
  END { if (!in_run) exit 2 }
' "${deploy_workflow}" > "${handoff_script}"
[ -s "${handoff_script}" ] \
  || qam_fail "foundation administrator handoff script could not be extracted"
mkdir -p "${handoff_test_dir}/quickagenticmemory/scripts"
for handoff_stub in deploy.sh validate-github-access.sh grant-fabric-access.sh; do
  cat > "${handoff_test_dir}/quickagenticmemory/scripts/${handoff_stub}" <<'HANDOFF_STUB'
#!/usr/bin/env bash
printf '%s\n' "$0 $*" >> "${QAM_HANDOFF_EXECUTION_LOG:?}"
exit 97
HANDOFF_STUB
  chmod +x "${handoff_test_dir}/quickagenticmemory/scripts/${handoff_stub}"
done
: > "${handoff_execution_log}"
(
  cd "${handoff_test_dir}"
  export QAM_GITHUB_AUTH_MODE=none
  export QAM_WORKFLOW_PRIVATE_NETWORKING=false
  export GITHUB_SHA=1111111111111111111111111111111111111111
  export GITHUB_STEP_SUMMARY="${handoff_summary}"
  export QAM_HANDOFF_EXECUTION_LOG="${handoff_execution_log}"
  bash -e -o pipefail "${handoff_script}"
)
[ ! -s "${handoff_execution_log}" ] \
  || qam_fail "foundation handoff executed a command that must only be rendered as Markdown"
[ "$(grep -c -- '--include-role-assignments' "${handoff_summary}")" -eq 1 ] \
  || qam_fail "administrator role-assignment command must appear exactly once in the rendered handoff"
grep -- '--include-role-assignments' "${handoff_summary}" | grep -Fq -- '--skip-app --include-role-assignments' \
  || qam_fail "rendered administrator handoff must preserve the guarded role-assignment command"
...
image_build_line="$(grep -n 'Build immutable image in ACR from exact public commit' "${deploy_workflow}" | cut -d: -f1)"
application_deploy_line="$(grep -n 'Deploy MCP application' "${deploy_workflow}" | cut -d: -f1)"
...
cloud_build_block="$(sed -n "${image_build_line},$((application_deploy_line - 1))p" "${deploy_workflow}")"
# shellcheck disable=SC2016 # These are intentionally literal workflow/runtime variables.
if ! grep -Fq 'build-cloud-image.sh "${args[@]}"' <<< "${cloud_build_block}" \
  || ! grep -Fq -- '--source-git-url "${GITHUB_SERVER_URL}/${GITHUB_REPOSITORY}.git"' <<< "${cloud_build_block}" \
  || ! grep -Fq -- '--source-ref "${GITHUB_SHA}"' <<< "${cloud_build_block}" \
  || ! grep -Fq -- '--expected-existing-digest' <<< "${cloud_build_block}" \
  || ! grep -Fq '.image.digest | test("^sha256:[0-9a-f]{64}$")' <<< "${cloud_build_block}" \
  || ! grep -Fq '.temporaryWriter.requested == false' <<< "${cloud_build_block}"; then
  qam_fail "protected deploy workflow must verify the exact public ACR cloud-build receipt before ACA"
fi
if grep -Fq 'build-push-image.sh' <<< "${cloud_build_block}" \
  || grep -Fq -- '--temporary-writer-principal-' <<< "${cloud_build_block}"; then
  qam_fail "protected deploy workflow must rely on its existing OIDC writer and never local Docker or self-granted RBAC"
fi
