...
curl() {
  ...
  printf '%s %s connect=%s max=%s\n' \
    "${method}" "${url}" "${connect_timeout}" "${max_time}" \
...
grep -Fxq "POST ${refresh_url} connect=2 max=5" "${success_log}" \
  || qam_fail "Fabric Graph refresh did not call the official start endpoint with hard curl timeouts"
[ "$(grep -Fxc "GET ${job_url} connect=2 max=5" "${success_log}")" -eq 2 ] \
