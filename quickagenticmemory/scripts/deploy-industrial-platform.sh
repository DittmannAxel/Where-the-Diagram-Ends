...
[ "$(printf '%s' "${runtime_principal_id}" | tr '[:upper:]' '[:lower:]')" != \
  "$(printf '%s' "${deployment_principal_id}" | tr '[:upper:]' '[:lower:]')" ] \
...
qam_info "applying the explicit Fabric Graph Preview runtime workaround"
...
jq -e \
  ...
  --arg deploymentPrincipalId "${deployment_principal_id}" '
    .runtimePrincipalId == $runtimePrincipalId and
    .runtimeMinimumRole == "Contributor" and
    (.runtimeObservedRole == "Contributor" or
      .runtimeObservedRole == "Member" or
      .runtimeObservedRole == "Admin") and
    .smokePrincipalId == $deploymentPrincipalId and
    .smokeMinimumRole == "Contributor" and
    (.smokeObservedRole == "Contributor" or
      .smokeObservedRole == "Member" or
      .smokeObservedRole == "Admin") and
    .definitionUpdaterPrincipalId == $deploymentPrincipalId and
    .definitionUpdaterMinimumRole == "Contributor" and
    .definitionUpdaterObservedRole == .smokeObservedRole
  ' <<< "${access_outputs}" >/dev/null \
  || qam_fail "Fabric access receipt does not prove the reviewed industrial role contract"
