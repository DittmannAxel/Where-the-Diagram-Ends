...
After creating the foundation and the two distinct managed identities described in the [infrastructure guide](../../infra/README.md), an authorized operator can deploy or reconcile the paid platform, Fabric items, least-privilege workspace roles, and a real Foundry inference in one command:
...
This command intentionally has no mandatory `what-if` step. It uses Bicep for the F capacity and Foundry account/project/model, then the public Fabric APIs for the isolated Workspace, Lakehouse, Graph Model, and checked-in Notebook. A repeated run must reuse the exact-name resources and roles.
