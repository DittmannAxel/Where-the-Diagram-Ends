...
4. A Fabric notebook validates the immutable pair and writes the `QamNode` and `QamEdge` Delta tables. An existing GraphModel is then mapped, saved, and refreshed manually only after both writes succeed.
5. A published Microsoft Foundry Agent Application calls the MCP gateway over Entra-authenticated HTTPS with its distinct application identity.
...
| Published Agent Application `defaultInstanceIdentity` | Outbound caller from Foundry to MCP. Its client/principal pair is live-verified, allowlisted by Container Apps EasyAuth, and assigned only the MCP API's `Qam.Read` application role. |
...
| Container App runtime UAMI | Acquires Fabric tokens and accesses only its required Key Vault secrets and telemetry destination. It is neither the Agent Application caller nor the inbound invoker. |
...
An unpublished Foundry project agent is not authorized as the production caller because it uses a project-shared development identity. The bootstrap first publishes an inert, tool-less Agent Application, reads its distinct `properties.defaultInstanceIdentity`, configures outbound access, and only then attaches the MCP-enabled version. See the [Foundry integration guide](../agents/foundry/README.md).
...
3. An authorized administrator configures the GitHub App PEM secret and exact selected-repository permission, grants the runtime UAMI the required Fabric workspace access, and prepares the existing Lakehouse, notebook, and GraphModel.
4. Publish the inert Foundry Agent Application, live-verify its distinct `defaultInstanceIdentity`, assign `Qam.Read`, and use its client/principal pair for both EasyAuth allowlists. Assign an inbound smoke-test identity `Foundry User` only at the published application scope.
5. Publish the verified projection to OneLake, run the notebook, and manually map/save/refresh the existing GraphModel only after the complete table write succeeds. Run the commit-pinned Fabric GQL smoke before application deployment.
...
- **Use least privilege:** GitHub deployment uses federated OIDC, the runtime uses its own UAMI, the GitHub App is selected-repository `Contents: read`, MCP tools are read-only, and the Fabric adapter exposes fixed operations instead of arbitrary GQL.
...
| Security | Entra authentication, GitHub OIDC, managed identity, Key Vault references, fixed read-only MCP operations, SHA pinning, and no arbitrary graph query. | External Container Apps ingress is needed for Foundry connectivity. Add private networking or an approved API perimeter where the tenant topology supports it, plus Defender, Policy, PIM, and SIEM controls. |
...
- The supplied automation stages validated files and updates existing Fabric items, but an authorized operator still owns the GraphModel mapping, save, and refresh gate.
